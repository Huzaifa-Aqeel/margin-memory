...
const checks:[string,()=>Promise<void>][]=[['Agent model',async()=>{
 ...
 const agent=new Agent({model:new BedrockModel({region:process.env.AWS_REGION||'us-east-1',modelId:process.env.BEDROCK_MODEL_ID,maxTokens:32}),printer:false})
 await agent.invoke('Reply with OK.',{limits:{turns:1,outputTokens:32},cancelSignal:AbortSignal.timeout(30000)})
}]]
