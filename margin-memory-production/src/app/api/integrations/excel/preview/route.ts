...
export async function POST(request:Request){
 ...
 try{
  ...
  if(!canImport){
   await createExcelIntegrationRun({id:runId,sourceIdentityHash:analyzed.source.sourceIdentityHash,snapshotHash:analyzed.source.snapshotHash,adapterVersion:analyzed.source.snapshot.adapterVersion,capturedAt:analyzed.source.snapshot.capturedAt,status:'failed',failureStage:'inspection',errorMessage:analyzed.analysis.issues.filter(issue=>issue.severity==='error').map(issue=>issue.message).join(' ')||'Workbook inspection was blocked.'})
  }else{
   ...
   await createExcelIntegrationRun({id:runId,sourceIdentityHash:analyzed.source.sourceIdentityHash,snapshotHash:analyzed.source.snapshotHash,adapterVersion:analyzed.source.snapshot.adapterVersion,capturedAt:analyzed.source.snapshot.capturedAt,status:'ready',reviewId})
  }
