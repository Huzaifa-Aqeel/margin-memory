...
export default async function InboxPage(){
  ...
  return <div className="page"><div className="page-head"><div><div className="eyebrow">Human judgment inbox</div><h1>Only the things the system can’t safely decide</h1><p className="subtle">Questions before submission, and verification after the job. {total?`${total} item${total===1?'':'s'} need attention.`:'Nothing is waiting.'}</p></div></div>
    ...
    <section className="section"><div className="section-head"><h2>Warning outcomes to verify</h2><Badge tone={outcomes.length?'pending':'ready'}>{outcomes.length} pending</Badge></div><div className="list">{outcomes.length?outcomes.map(({estimate,outcome,finding})=><div className="card" key={outcome.id}><div className="finding-top"><div><div className="eyebrow">{estimate.name}</div><h3>{finding?.title??'Margin Check warning'}</h3></div><Link href={`/estimates/${estimate.id}`} className="btn small">Open closeout</Link></div><p>{outcome.explanation}</p><div className="rationale">{outcome.evidenceSummary}</div><div style={{marginTop:12}}><WarningResponseHistory responses={finding?.responses}/><FindingOutcomeActions outcomeId={outcome.id} responses={finding?.responses??[]}/></div></div>):<div className="card empty">No warning outcomes need confirmation.</div>}</div></section>
