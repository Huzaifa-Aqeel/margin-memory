...
import { automaticExcelSourceCandidate, canonicalWorkbookLocation, EXCEL_SOURCE_ADAPTER_VERSION, type ExcelLiveSnapshot, type ExcelSourceSelection, type ExcelSnapshotCell } from '@/lib/integrations/excel-snapshot'
...
const get=async <T,>(url:string):Promise<T>=>{const response=await fetch(url,{cache:'no-store'});const data=await response.json() as T&{error?:string};if(!response.ok)throw new Error(data.error||`Request failed (${response.status}).`);return data}
const presentationState=(presentation:ExcelEstimatePresentation):PaneState=>presentation.result==='needs_input'?'needs_input':presentation.result==='findings'?'findings':presentation.result==='no_findings'?'no_findings':presentation.result==='failed'?'error':'running'
...
export function ExcelTaskPane({authenticated,workspaceName}:{authenticated:boolean;workspaceName?:string}){
 ...
 const inspectWorkbook=useCallback(async()=>{try{setState('inspecting');setError('');const discovered=await discoverSources();setChoices(discovered);const automatic=automaticExcelSourceCandidate(discovered);if(automatic)return inspectChoice(automatic);setState('source_selection')}catch(cause){setError(cause instanceof Error?cause.message:'Could not inspect this workbook.');setState('error')}},[inspectChoice])

 const runCheck=async()=>{if(!choice||!snapshot||!preview?.review)return;try{setState('running');setError('');const current=await captureSource(choice);const response=await post<{estimate:ExcelEstimatePresentation}>('/api/integrations/excel/check',{snapshot:current,reviewId:preview.review.id,reportHash:preview.review.reportHash,runId:preview.runId,reviewed});setResult(response.estimate);setState(presentationState(response.estimate))}catch(cause){setError(cause instanceof Error?cause.message:'Margin Check failed.');setState('error')}}
 const answerQuestion=async(questionId:string,answer:string)=>{if(!result)return;try{setState('running');const response=await post<{estimate:ExcelEstimatePresentation}>(`/api/integrations/excel/estimates/${result.estimateId}`,{questionId,answer});setResult(response.estimate);setState(presentationState(response.estimate))}catch(cause){setError(cause instanceof Error?cause.message:'Could not resume the review.');setState('error')}}
 ...
 useEffect(()=>{if(state!=='running'||!result?.estimateId)return;const timer=window.setInterval(async()=>{try{const response=await get<{estimate:ExcelEstimatePresentation}>(`/api/integrations/excel/estimates/${result.estimateId}`);setResult(response.estimate);setState(presentationState(response.estimate))}catch(cause){setError(cause instanceof Error?cause.message:'Could not refresh the running review.');setState('error')}},2000);return()=>window.clearInterval(timer)},[state,result?.estimateId])
 ...
 return <div className="excel-pane">
  ...
  {(state==='inspecting'||state==='running')&&<section className="excel-state centered"><LoaderCircle className="spin" size={30}/><h2>{state==='inspecting'?'Inspecting selected source':result?.investigationStatus==='queued'?'Margin Check is waiting to run.':'Running Margin Check…'}</h2><p>{state==='inspecting'?'Applying the same import checks used by Margin Memory.':result?.investigationStatus==='queued'?'Your estimate has not been cleared. The review will begin when processing is available.':'Checking the estimate against verified company history and recorded calculations.'}</p></section>}
