...
export function UploadEstimateForm({parentEstimateId}:{parentEstimateId?:string}){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!previewReady){setError('Preview the selected estimate and review any limitations before starting the preflight.');return}setBusy(true);setError('');const form=new FormData(event.currentTarget);const res=await fetch('/api/estimates/review',{method:'POST',body:form});const data=await res.json();if(!res.ok){setError(data.error||'Could not review estimate.');setBusy(false);return}const warning=(data.warnings as string[]|undefined)?.join(' ');router.push(`/estimates/${data.estimate.id}${warning?`?warning=${encodeURIComponent(warning)}`:''}`);router.refresh()}
  return <form ref={formRef} className="card form-card" onSubmit={submit}>
    <div className="form-grid">
      ...
      <div className="field full"><label>Project documents (optional)</label><div className="file-drop"><input name="projectDocuments" type="file" accept=".pdf,.txt,.md" multiple onChange={fileChanged}/><div className="helper">Up to 5 PDFs/TXT/MD files. Text is extracted server-side so the agent can inspect addenda, schedules, scope notes, and specifications.</div></div></div><div className="field full"><label>Assumptions / exclusions</label><textarea name="assumptions" placeholder={'One per line\nExisting conduit can be reused where accessible\nNormal daytime access to work areas'}/><div className="helper">These are especially useful to the agent. Paste the assumptions you would normally carry in the bid.</div></div>
    </div>
    {error&&<div className="error" style={{marginTop:14}}>{error}</div>}
    <div className="actions" style={{marginTop:18}}><button className="btn primary" disabled={busy}><SearchCheck size={16}/>{busy?'Agent is investigating…':'Review estimate'}</button></div>
  </form>
}
