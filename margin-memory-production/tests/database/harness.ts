...
export async function startDatabase(){
 ...
 try{
  ...
  for(const migration of readdirSync('supabase/migrations').filter(f=>f.endsWith('.sql')).sort())await db.query(readFileSync(join('supabase/migrations',migration),'utf8'))
