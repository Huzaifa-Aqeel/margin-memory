...
import {AgentOutputSchema} from '../src/lib/agent/provenance'
...
it('loads the Bedrock investigator without OpenAI or optional S3 integrations',()=>{const model=new BedrockModel({region:'us-east-1',modelId:'amazon.nova-lite-v1:0'});const agent=new Agent({model,printer:false,structuredOutputSchema:AgentOutputSchema});expect(agent).toBeInstanceOf(Agent)})
