...
it('uses the reviewed API preview interpretation for committed database values',async()=>{
 ...
 const previewForm=form();previewForm.set('mode','pair')
 ...
 const switched=form();switched.set('actualFile',new File([actualCsv.replace('1300','9300')],'reviewed-actual.csv',{type:'text/csv'}));switched.set('name','Substitution');switched.set('completedAt','2026-08-01');switched.set('scopeReview',JSON.stringify({status:'no_changes',changes:[]}));switched.set('importReviewId',preview.review.id);switched.set('importReportHash',preview.review.reportHash);switched.set('importReviewed','true');switched.set('actualCompletenessConfirmed','true')
 expect((await commitHistoricalImport(new Request('http://localhost/api/jobs/import',{method:'POST',body:switched}))).status).toBe(409)
 const tampered=form();tampered.set('name','Tampered');tampered.set('completedAt','2026-08-01');tampered.set('scopeReview',JSON.stringify({status:'no_changes',changes:[]}));tampered.set('importReviewId',preview.review.id);tampered.set('importReportHash','0'.repeat(64));tampered.set('importReviewed','true');tampered.set('actualCompletenessConfirmed','true')
 ...
 commitForm.set('name','API import parity')
 commitForm.set('completedAt','2026-08-01')
 ...
 const sourceTrace=(await database.db.query('select p.source_row,p.worksheet,f.file_name,p.original_values from public.import_line_provenance p join public.import_review_files f on f.id=p.import_review_file_id where p.job_actual_line_id is not null and p.organization_id=$1 order by p.source_row',[orgA])).rows
 ...
 const replay=form();replay.set('name','Changed on retry');replay.set('completedAt','2026-08-01');replay.set('scopeReview',JSON.stringify({status:'no_changes',changes:[]}));replay.set('importReviewId',preview.review.id);replay.set('importReportHash',preview.review.reportHash);replay.set('importReviewed','true');replay.set('actualCompletenessConfirmed','true')
